...
async function compileLocales() {
  ...
  for (const dirent of dirents) {
    ...
    await writeFile(path.join('./dist/locales', dirent.name + '.json'), JSON.stringify(dist, undefined, 0), 'utf8');
  ...
  try {
    await writeFile('./dist/locales.json', JSON.stringify(locales, undefined, 0), 'utf8');
  } catch (error) {
    console.error(error);
  }
}
